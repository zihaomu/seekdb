...
#ifndef OCEANBASE_QUERY_VECTOR_INDEX_UTIL_H_
#define OCEANBASE_QUERY_VECTOR_INDEX_UTIL_H_
...
static constexpr double DEFAULT_IVFPQ_SELECTIVITY_RATE = 0.9;
  ...
  int set_vec_param_info(const schema::ObTableSchema *vec_index_schema);
...
public:
  ...
  static bool should_set_max_lob_inrow_threshold_for_async_index(
      const schema::ObTableSchema &tbl_schema,
      const schema::ObIndexType vec_index_type,
      const ObString &index_params);
  static bool is_sync_mode_async(const ObString &index_params);
  static bool is_sync_mode_async(
      const ObString &index_params,
      bool is_hnsw_heap_table);
  ...
  static int get_vector_index_column_dim(const schema::ObTableSchema &index_table_schema, int64_t &dim);
  ...
  static int get_vector_index_type(
      ...
      ObIArray<schema::ObIndexType> &type_array);
  ...
  static int update_index_tables_status(
      ...
      ObIArray<schema::ObTableSchema> &table_schemas);
  ...
  static int construct_new_column_schema_from_exist(
      const schema::ObColumnSchemaV2 *old_column_ptr,
      const schema::ObColumnSchemaV2 *&new_column_ptr,
      const VecColType col_type,
      schema::ObColumnSchemaV2 &new_column,
      uint64_t &available_col_id);
  static int set_new_index_column(
      schema::ObTableSchema &new_index_schema,
      const schema::ObColumnSchemaV2 *old_column_ptr,
      const schema::ObColumnSchemaV2 *&new_column_ptr);
  ...
  static int get_ivf_aux_info(query::ObIVectorIndexService *service,
  ...
  static bool check_ivf_vector_index_memory(schema::ObSchemaGetterGuard &schema_guard, const schema::ObTableSchema &index_schema, const int64_t row_count);
  static int estimate_vector_memory_used(
      schema::ObSchemaGetterGuard &schema_guard,
      const schema::ObTableSchema &index_schema,
      const int64_t tablet_row_count,
      int64_t &estimate_memory);
  static int alter_vec_aux_column_schema(const schema::ObTableSchema &aux_table_schema,
                                         const schema::ObColumnSchemaV2 &new_column_schema,
                                         schema::ObColumnSchemaV2 &new_aux_column_schema);
  static int set_vector_index_param(const schema::ObTableSchema *&vec_index_schema,
  ...
  static bool is_sindi_index(const schema::ObTableSchema *vec_index_schema);
...
private:
  static void save_column_schema(
      const schema::ObColumnSchemaV2 *&old_column,
      const schema::ObColumnSchemaV2 *&new_column,
      const schema::ObColumnSchemaV2 *cur_column);
  static int check_index_param(
      ...
      ObString &index_params,
      schema::ObIndexType &out_index_type,
      const schema::ObTableSchema &tbl_schema,
      sql::ObSQLSessionInfo *session_info);
  ...
  static int has_same_cascaded_col_id(
      const schema::ObTableSchema &data_table_schema,
      const schema::ObColumnSchemaV2 &col_schema,
      const int64_t col_id,
      bool &has_same_col_id);
  static bool check_is_match_index_type(
      const schema::ObIndexType type1, const schema::ObIndexType type2);
...
#endif // OCEANBASE_QUERY_VECTOR_INDEX_UTIL_H_
